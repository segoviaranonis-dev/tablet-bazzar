# ACTUAL — 2026-08-11

**Actualizado:** 2026-08-11 · CSV precios Tito **2.3.1.7.5.3.17** · Bancard Laura ⏳  
**Línea 1 agente:** Si pienso en el lo entiendo, pero si me lo explicarlo es imposible · `5.01.00.025`

---

## FOCO ahora

| Tema | Código | Doc / app |
|------|--------|-----------|
| **CSV precios Tito · PP tab FI** | **2.3.1.7.5.3.17** | Botón 💰 · no pisa CSV Carlos · [CHUSAR](../2_modulos/2.3_report/proceso_importacion/CHUSAR_CSV_PRECIOS_TITO_PP_20260811.md) |
| **Aprobación Gral · molécula** | **2.3.1.3.7** | Botón en tarjeta · solo FI de ese PVR · `:3000/aprobaciones` |
| **Bancard · Laura Vera · aguardando** | **2.5.1.32** | `CHUSAR_BANCARD_CONTACTO_LAURA_STANDBY_FOTO_20260811.md` |
| Standby **cambio de foto** | **2.5.1.24** / `2.01.04.024` | Keyword **imagen de portada** · batch listo |
| 2ª integridad · Bancard + EDB | **2.5.1.30** | Stock → pedido1 → Bancard → EDB |
| Mapa carrito PE → Bazzar | **2.5.1.31** | Héctor · validación OK |
| EDB / Bóveda FI logística | **2.7.2** · **2.5.1.27** | `:3005` · `:3000/bazzar-web/bobeda-oro` |

**Bancard:** contacto WhatsApp Laura Vera abierto — **no integrar** hasta respuesta + claves.  
**Foto:** pipeline portada listo; esperar archivos + keyword **imagen de portada**.  
**Siglas:** **EDB** = Empresa de Delivery de Bazzar.

---

## Keyword viva

**Protocolo chusar activado** · lotes Moises · sync OPS **OFF** · zip Héctor.
