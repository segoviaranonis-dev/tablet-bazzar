# ETAPA — Final de Bazzar Web · lanzamiento 01-09-2026

**ID:** `FINAL-BAZZAR-WEB-20260806`  
**Alias:** Final de Bazzar Web · pulir hasta lanzamiento  
**Estado:** 🟢 **EN CURSO · FOCO** · apertura 2026-08-06 (orden Director: **abre la etapa** · **Documenta**)  
**Meta lanzamiento:** **2026-09-01**  
**Módulo:** Bazzar Web (`bazzar-web/` · :3002 · prod https://www.bazzar.com.py)  
**Ejecutor:** Cursor (+ Claude/Gemini según OT)  
**Shibboleth:** Andrés, el que viene. CHUNA activo · Moria + ACTUAL acatados.

**CHUSAR:** [CHUSAR_FINAL_BAZZAR_WEB_LANZAMIENTO_20260901.md](../2_modulos/2.5_bazzar_web/CHUSAR_FINAL_BAZZAR_WEB_LANZAMIENTO_20260901.md) (**2.5.1.18**)

---

## Norte Director

Terminar y **pulir** Bazzar Web hasta el **lanzamiento 01-09-2026**: catálogo, stock, grada 638, checkout, filtros siameses, integridad, deploy y smoke de go-live.

---

## Alcance (borrador operativo)

| # | Frente | Nota |
|---|--------|------|
| 1 | Catálogo / grilla / grada | Paridad Stock Sano · chips · qty |
| 2 | Filtros header siameses | Dimensión + molécula |
| 3 | Compra web / carrito / checkout | Cédula · `cliente_web` |
| 4 | Stock ALM_WEB / purge / TRP | Lecciones 2.5.1.13–17 |
| 5 | Motor precio / NIIF / imágenes | Ley Universal |
| 6 | Prod Vercel + dominio | https://www.bazzar.com.py |
| 7 | Smoke go-live pre 01-09 | Checklist cierre |

**Fuera (salvo orden):** Moises (pausada fin de semana) · oversell CP (espera SR) · deploy rimec-web no relacionado.

---

## Relación con etapas abiertas

| Etapa | Rol tras esta apertura |
|-------|------------------------|
| **FINAL-BAZZAR-WEB-20260806** | **FOCO** |
| `MOISES-20260804` (unificada + orilla) | Pausada · fin de semana / más tarde |
| `CP-OVERSELL-ACUSACION-20260805` | En espera SR · no FOCO |
| Cascada filtros rimec **2.2.1.42** | Hilo técnico paralelo (no tarjeta etapa) |

---

## Checklist etapa

| # | Ítem | Estado |
|---|------|:------:|
| 1 | Apertura Moria + `etapas.json` + :3004 | ✅ 2026-08-06 |
| 2 | Inventario gaps vs go-live | ⏳ |
| 3 | Pulido + smokes locales | ⏳ |
| 4 | Deploy / dominio listo | ⏳ |
| 5 | Cierre canónico pre/post 01-09 | ⏳ |

---

## Referencias

- Índice módulo: `.claude/2_modulos/2.5_bazzar_web/INDICE.md`  
- Adecuación senior: `CHUSAR_ADECUACION_BAZZAR_RIMEC_AUDITORIA_SENIOR_20260802.md` (**2.5.1.8**)  
- Deploy: `DEPLOY_VERCEL_BAZZAR_WEB.md`

---

**Apertura 2026-08-06 — FOCO holding hasta lanzamiento 01-09-2026.**
